...
object ChannelUtil {
    ...
    @Deprecated("使用 PlaybackUtil.isPlaybackSupported() 替代", ReplaceWith("PlaybackUtil.isPlaybackSupported(url)"))
    fun urlSupportPlayback(url: String): Boolean {
        return listOf("pltv", "tvod").any { url.contains(it, ignoreCase = true) }
    }

    @Deprecated("使用 PlaybackUtil.convertToPlaybackUrl() 替代", ReplaceWith("PlaybackUtil.convertToPlaybackUrl(url)"))
