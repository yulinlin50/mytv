...
fun SettingsCategoriesScreen(
    ...
    lastSelectedCategory: SettingsCategories? = null,
    onBackPressed: () -> Unit = {},
) {
    ...
    AppScreen(
        ...
        // 为每个分类创建一个FocusRequester
        val focusRequesters = remember { SettingsCategories.entries.map { FocusRequester() } }
        // 确定应该聚焦的分类：最后选中的分类或第一个分类
        val targetCategory = lastSelectedCategory ?: SettingsCategories.entries.first()
        val targetIndex = SettingsCategories.entries.indexOf(targetCategory)
        // 标记是否可以处理点击事件
        ...
        LazyVerticalGrid(
            ...
            itemsIndexed(SettingsCategories.entries) { index, category ->
                SettingsCategoryItem(
                    ...
                    enabled = canHandleClick,
                    onSelected = { toSettingsCategoryScreen(category) },
                )
            }
        }
    }
...
private fun SettingsCategoryItem(
    ...
    Surface(
        modifier = modifier
            ...
            .handleKeyEvents(onSelect = { if (enabled) onSelected() }),
