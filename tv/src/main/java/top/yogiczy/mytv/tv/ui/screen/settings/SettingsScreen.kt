...
fun SettingsScreen(
    ...
    // 记录最后选中的分类，用于返回时恢复焦点
    var lastSelectedCategory by remember { mutableStateOf<SettingsCategories?>(null) }

    AppScreen(
        modifier = modifier,
        onBackPressed = {
            // 优先处理内层导航返回，如果当前不在分类页面则返回上级
        ...
        NavHost(
            ...
            builder = {
                ...
                composable(SettingsCategories.EPG.name) {
                    SettingsEpgScreen(
                        toIptvSourceScreen = {
                            navController.navigateSingleTop(SettingsSubCategories.IPTV_SOURCE.name)
                        },
                        toEpgSourceScreen = {
                            navController.navigateSingleTop(SettingsSubCategories.EPG_SOURCE.name)
                        },
                        onBackPressed = { navController.navigateUp() },
                    )
                }
                ...
                composable(SettingsSubCategories.IPTV_SOURCE.name) {
                    SettingsIptvSourceScreen(
                        ...
                        onIptvSourceListChange = { settingsViewModel.iptvSourceList = it },
                        onAddIptvSource = { newSource ->
                            settingsViewModel.iptvSourceList = 
                                IptvSourceList(settingsViewModel.iptvSourceList + newSource.copy(enabled = true))
                            onReload()
                        },
                        onUpdateIptvSource = { updatedSource ->
                            settingsViewModel.iptvSourceList = IptvSourceList(
                                settingsViewModel.iptvSourceList.map {
                                    if (it.stableId() == updatedSource.stableId()) updatedSource else it
                                }
                            )
                        },
                        onDeleteIptvSource = {
                ...
                composable(SettingsSubCategories.EPG_SOURCE.name) {
                    SettingsEpgSourceScreen(
                        epgSourceListProvider = { settingsViewModel.epgSourceList },
                        onEpgSourceListChange = { settingsViewModel.epgSourceList = it },
                        onAddEpgSource = { newSource ->
                            settingsViewModel.epgSourceList = 
                                EpgSourceList(settingsViewModel.epgSourceList + newSource)
                        },
                        onUpdateEpgSource = { updatedSource ->
                            settingsViewModel.epgSourceList = EpgSourceList(
                                settingsViewModel.epgSourceList.map {
                                    if (it.url == updatedSource.url && it.name == updatedSource.name) updatedSource else it
                                }
                            )
                        },
                        onDeleteEpgSource = {
