...
object HttpServerSecurity {
    ...
    private const val KEY_TOKEN_CREATED_AT = "http_server_token_created_at"
    private const val KEY_MIGRATION_DONE = "http_server_token_migration_done"
    private const val MAX_UPLOAD_SIZE = 100 * 1024 * 1024L
    private const val ENCRYPTED_PREFS_NAME = "secure_http_server_prefs"
    private const val TOKEN_EXPIRY_MS = 60 * 60 * 1000L
    ...
    private fun isTokenExpired(context: Context): Boolean {
        val createdAt = getEncryptedPrefs(context).getLong(KEY_TOKEN_CREATED_AT, 0)
    ...
    fun getAccessToken(context: Context): String {
        ...
        if (_accessToken != null && !isTokenExpired(context)) {
            return _accessToken!!
        }
    ...
    fun validateToken(authHeader: String?, context: Context): Boolean {
        ...
        if (isTokenExpired(context)) return false
